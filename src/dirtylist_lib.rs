...
pub fn perform_simulation(
  ...
  let mut traces: Vec<Vec<NetState>> = vec![vec![NetState::Invalid]; nets_to_trace.len()];
  //let mut components_by_output_net: Vec<Vec<Component>> =
  //  (0..net_count).into_iter().map(|x| Vec::new()).collect();
  let mut components: Vec<Component> = Vec::new();
  let mut dirty_nets_bitmaps: [Vec<u64>; 2] = [vec![], vec![]];
  let mut dirty_swap: u8 = 0;
  let mut components_to_update_by_dirtied_net: Vec<Vec<u32>> = vec![Vec::new(); net_count as usize];
    ...
    while i < description.len() {
      match description[i] {
        1 => {
          ...
          // Mark that if gate or source update then this net must.
          components_to_update_by_dirtied_net[gate as usize].push(components.len() as u32);
          components_to_update_by_dirtied_net[source as usize].push(components.len() as u32);
